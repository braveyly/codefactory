...
ACCEPT_AGAIN:
                if ( -1 == ( connfd = accept ( listenfd, ( SA * ) & cliaddr, &sinsize ) ) )
                {
                    ...
                    {
                       printf( "accept error for non-blocking reason %s\n", strerror( errno ) );
		         goto ACCEPT_AGAIN;
                    	}
			else
			{
			   printf( "accept error for no non-blocking reason %s\n", strerror( errno ) ); 
			   return -1;
			}
                }
                else
            ...
            if ( actualfd != listenfd )
            {
                ...
                if ( 0 > ( recvlen = recv ( connfd, buf, buflen, 0 ) ) )
                {
			 ...
			 else
			 {
                ...
                if ( 0 > ( sendlen = send ( connfd, buf, strlen ( buf ) + 1, 0 ) ) )
                {
